...
exports.sendEmails = function(subscription, errorCallback) {
    ...
    pg.connect(process.env.DATABASE_URL || config.db_connection_string, function(err, client) {
        ...
        query.on('error', function(err) {
            errorCallback("get all subscriptions error = " + err);
        });
        ...
        query.on('end', function(results) {
            getIncidents(client, subscriptions, errorCallback);
        });
...
var getIncidents = function(client, subscriptions, errorCallback) {
    ...
    var getNeighborhoodIncidents = function(subscription, callback) {
        ...
        if (!neighborhoodIncidents[neighborhood]) {
            ...
            query.on('error', function(err) {
                errorCallback("get neighborhood incidents query error = " + err);
            })
    ...
    async.forEach(subscriptions, getNeighborhoodIncidents, function(err) {
        ...
        if (err) {
            errorCallback('get neighborhood incidents error = ' + err);
        }

        sendEmails(subscriptions, oneWeekAgo, today, neighborhoodIncidents, errorCallback);
    });

}

var sendEmails = function(subscriptions, startDate, endDate, neighborhoodIncidents, errorCallback) {
    ...
    var sendEmail = function(subscription, callback) {
        ...
        var emailHtml = '<html>'
            + '<body>'
            + '<img src="' + config.app_base_uri + 'images/logo-retina.png" width="740px" />'
    ...
    async.forEach(subscriptions, sendEmail, function(err) {
        if (err) {
            errorCallback("send emails error = " + err);
        }
    });
