...
var sendEmails = function(subscriptions, startDate, endDate, neighborhoodIncidents, emailSenderCallback) {
    ...
    var sendEmail = function(subscription, callback) {
        ...
        var emailHtml = '<html>'
            + '<body>'
            + '<img src="' + config.app_base_uri + 'images/logo-retina.png" width="740" height="31" />'
            + '<h1 style="padding: 0; font-size: 150%; line-height: 160%; color: #95a882">Crime incidents in ' + neighborhood + '<br>' + dateRange + '</h1>'

        // Generate list and map, if necessary
        if (incidents.length > 0) {
            
            emailHtml += '<br><br><table width="100%">'
                + '<thead>'
                + '  <tr>'
            ...
            // TODO: This will break if there are too many incidents
            for (index in incidents) {
                var incident = incidents[index];
        ...
        emailHtml += '<p style="color: gray"><br><br><br>Louisville Neighborhood Crime Data<br>'
            + '<a style="color: gray" href="mailto:louisville@codeforamerica.org">Contact us</a> '
            + '· <a style="color: gray" href="' + config.app_base_uri + '">Visit the website</a> '
            + '· <a style="color: gray" href="' + unsubscriptionLink + '">Unsubscribe from ' + neighborhood + ' emails</a> '
            + '· <a style="color: gray" href="' + userUnsubscriptionLink + '">Unsubscribe from all emails</a></p>'
            + '</body>'
            + '</html>';
